@echo off
rem ============================================================
rem  Start DSH Web GUI with the my-plugin patch loaded.
rem  Target: http://127.0.0.1:3080
rem  If the server is already running, just open the browser and
rem  remind you to restart with --patch to pick up my-plugin.
rem ============================================================
setlocal
...
if %errorlevel%==0 (
  ...
  echo [DSH] It was NOT started with --patch, so my-plugin is not loaded.
  echo [DSH] Close the existing server window and run this script again.
  start "" "%URL%"
  exit /b 1
)

rem --- 2. Start the server with the my-plugin overlay in its own console window ---
start "DeepSeek Harness Server (my-plugin)" /D "%~dp0" cmd /k ""%NODE%" "%CLI%" web --patch "%PATCH%""
...
if not %errorlevel%==0 (
  ...
  echo [DSH] "DeepSeek Harness Server (my-plugin)" window for logs.
